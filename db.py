...
class PostgresConnection:
    ...
    def execute(self, sql, params=None):
        cursor = self.raw_connection.cursor(cursor_factory=DictCursor)
        cursor.execute(_postgres_sql(sql), params or ())
        return cursor
...
def insert_id(con, sql, params):
    """Insert a row and return its generated primary key on both database engines."""
    if using_postgres():
...
def init_db():
    with connection() as con:
        ...
        if using_postgres():
            schema = (
                schema.replace("INTEGER PRIMARY KEY AUTOINCREMENT", "INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY")
                .replace("base_points INTEGER NOT NULL DEFAULT 0", "base_points DOUBLE PRECISION NOT NULL DEFAULT 0")
                .replace("points INTEGER NOT NULL DEFAULT 0", "points DOUBLE PRECISION NOT NULL DEFAULT 0")
                .replace("points_suggestion INTEGER", "points_suggestion DOUBLE PRECISION")
        ...
        if con.execute("SELECT COUNT(*) n FROM activity_types").fetchone()["n"] == 0:
            ...
            con.executemany(
                ...
                [
                    (presence_field_id, "Halve dag", 3),
                    (presence_field_id, "Hele dag", 6),
...
def get_tasks(include_past=False):
    if include_past:
        ...
        where = "WHERE t.task_date::date>=CURRENT_DATE AND t.completed=0"
    else:
        where = "WHERE date(t.task_date)>=date('now') AND t.completed=0"
    with connection() as con:
        tasks = rows(
            con.execute(
                """
